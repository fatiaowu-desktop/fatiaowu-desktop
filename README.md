...
> 用 macOS 原生 WebKit 包裹 DeepSeek Harness Web 界面，配三套手搓皮肤、余额监控、表情面板、快捷键换肤、离线语音输入与朗读，以及一屋子活物：会变速的发条机芯，和一只会发光的小鲸鱼。
...
| 🎛 设置内换肤 | 设置面板内置「皮肤」栏目，三套皮肤带迷你预览，点击即换（或 ⌥⌘1/2/3） |
| 📊 会话统计 | 底部完整显示轮数/耗时/token 统计 |
| 🌋 动态皮肤 | 猩红熔岩粒子 + 烟尘 + 光晕呼吸；暗金工作区萤火虫星尘（全部无缝循环） |
| 🎙 语音输入 | **设备端离线识别**（免费、音频不出本机）；唤醒词「小鲸鱼 / 小金鱼」，说完自动发送；能量门控 + 停顿判句 + 接续窗口 |
| 🔊 回复朗读 | 答完自动念（可关）；开口即打断朗读；说完 8 秒内可接着说，不用再喊唤醒词 |
| ⚙️ 发条机芯 | 侧栏底部三只咬合齿轮，随 AI 运转状态实时变速（怠速常转 = 生命体征）；仪表显示**真实 token**，非估算 |
| 🐋 小鲸鱼桌宠 | 会游动、躲鼠标、探头好奇、双击投食、追泡泡、静置打盹；可拖拽、可甩出去；AI 完成时替你庆祝 |
| 🎵 机械音效 | Web Audio 程序化合成（上弦回弹「咔哒」等），**零音频文件** |
| 🧬 抗升级锚点 | 状态判据走语义锚点（统计胶囊 / aria-label / 文案），不依赖 CSS-Module 哈希类名 |
